class Meetup
  ...
  def day(weekday, schedule)
    ...
    # `.detect` would be same as `.each` or a similar Enumerator iterator due to
    # `break` statement within the block -- This comment ended up not being true
    # because nil is returned when detect iterates through each argument
    # and does not find a true statement
